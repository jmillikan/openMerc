...
void npc_cityattack_low(int cn)
{
	...
	{
	case    0:
		ret = npc_moveto(cn, 456, 356);
		break;
	case    1:
		ret = npc_moveto(cn, 447, 356);
		break;
	case    2:
		ret = npc_moveto(cn, 447, 362);
		break;
	case    3:
		ret = npc_moveto(cn, 474, 362);
		break;
	case    4:
		ret = npc_cityattack_wait(cn);
		break;
	case    5:
		ret = npc_moveto(cn, 486, 362);
		break;
	case    6:
		ret = npc_moveto(cn, 509, 362);
		break;
	case    7:
		ret = npc_moveto(cn, 526, 362);
		break;
	case    8:
		ret = npc_moveto(cn, 531, 386);
		break;
	case    9:
		ret = npc_moveto(cn, 534, 403);
		break;
	}
	if (ret)
	{
