...
void answer_shop(int cn, int co)
{
	...
	{
		do_sayx(cn, "Hold down ALT and right click on me to buy or sell, %s.", ch[co].name);
	}
	else
	{
...
void answer_buygold(int cn, int co)
{
	...
	if (pts<1)
	{
		do_sayx(cn, "But you don't have any points to spend, %s!", ch[co].name);
		return;
	}
...
void answer_buyhealth(int cn, int co)
{
	...
	if (pts<6)
	{
		do_sayx(cn, "But you don't have enough points to spend, %s!", ch[co].name);
		return;
	}
...
void special_answer(int cn, int co, int spec, char *word, int nr)
{
	...
	case TIME:
		...
		break;
	case POINTS:
		answer_points(cn, co, nr);
		break;
	case BUYGOLD:
		answer_buygold(cn, co);
		break;
	case BUYHEALTH:
		answer_buyhealth(cn, co);
		break;
	case BUYMANA:
		answer_buymana(cn, co);
		break;
	case BUYEXP:
		answer_buyexp(cn, co);
		break;
	case TRANSFER:
		answer_transfer(cn, co);
		break;
	case SPELLINFO:
		answer_spellinfo(cn, co);
		break;
	case QUIET:
		answer_quiet(cn, co);
		break;
