...
void do_say(int cn, char *text)
{
	...
	{
		do_char_log(cn, 0, "Use #help instead.\n");
	}
	...
	if (map[m].flags & MF_UWATER)
	{
		for (n = 0; n<20; n++)
		{
			if ((in = ch[cn].spell[n])!=0 && it[in].temp==648)
			{
				break;                                          // blue pill
			}
		}
...
void do_char_killed(int cn, int co)
{
	...
	if (cn && cn!=co && (ch[cn].flags & (CF_PLAYER)) && !(mf & MF_ARENA))
	{
		...
		if (abs(r1 - r2)<3) // aprox. own rank
		{
			...
			if (ch[co].data[42]==27)
			{
				ch[cn].data[27]++;                      // black stronghold counter
			}
		}
...
//	y = ch[co].y;
	...
	else if (!(ch[co].flags & CF_LABKEEPER))
	{
		...
		ch[co].a_hp = 0;
#ifdef KILLERONLY
		if ((cc = ch[cn].data[63])!=0 && (ch[cc].flags & (CF_PLAYER)))
		{
...
int get_fight_skill(int cn)
{
	...
	{
		return( ch[cn].skill[SK_DAGGER][5]);
	}
	if (it[in].flags & IF_WP_AXE)
	{
		return( ch[cn].skill[SK_AXE][5]);
	}
	if (it[in].flags & IF_WP_STAFF)
	{
		return( ch[cn].skill[SK_STAFF][5]);
	}
...
int do_hurt(int cn, int co, int dam, int type)
{
	...
	if (ch[co].temp==CT_COMPANION && !(ch[co].flags & CF_THRALL))
	{
		noexp = 1;
	}
...
void do_attack(int cn, int co, int surround)
{
	...
	if (ch[co].stunned || !ch[co].attack_cn)
	{
		s2 -= 10;
	}
	...
	if (hit)
	{
		...
		if (ch[cn].attrib[AT_STREN][5]>3)
		{
			dam += RANDOM(ch[cn].attrib[AT_STREN][5] / 2);
		}
		if (die==2)
		{
			dam += RANDOM(6) + 1;
...
int do_char_can_see(int cn, int co)
{
	...
	if (ch[co].mode==0)
	{
		d = (d * (ch[co].skill[SK_STEALTH][5] + 20)) / 20;
	}
	else if (ch[co].mode==1)
	{
		d = (d * (ch[co].skill[SK_STEALTH][5] + 50)) / 50;
	}
	else
	{
		d = (d * (ch[co].skill[SK_STEALTH][5] + 100)) / 100;
	}
	...
	if (!can_see(cn, ch[cn].x, ch[cn].y, ch[co].x, ch[co].y, 15))
	{
		prof_stop(21, prof);
		return( 0);
	}
...
int do_char_can_see_item(int cn, int in)
{
	...
	if (!can_see(cn, ch[cn].x, ch[cn].y, it[in].x, it[in].y, 15))
	{
		prof_stop(22, prof);
		return( 0);
	}
...
void really_update_char(int cn)
{
	...
	for (n = 0; n<20; n++)
	{
		...
		if (!(ch[cn].flags & CF_NOMAGIC))
		{
			...
			for (z = 0; z<5; z++)
			{
				if (it[m].active)
				{
					attrib[z] += it[m].attrib[z][1];
				}
				else
				{
					attrib[z] += it[m].attrib[z][0];
				}
			}
			...
			for (z = 0; z<50; z++)
			{
				if (it[m].active)
				{
					skill[z] += it[m].skill[z][1];
				}
				else
				{
					skill[z] += it[m].skill[z][0];
				}
			}
		}
	...
	if (!(ch[cn].flags & CF_NOMAGIC))
	{
		for (n = 0; n<20; n++)
		{
			...
			if (it[m].temp==SK_STUN || it[m].temp==SK_WARCRY2)
			{
				ch[cn].stunned = 1;
			}
			...
			if (it[m].sprite_override)
			{
				ch[cn].sprite_override = it[m].sprite_override;
			}
...
// !!!!!        if ((ch[cn].flags&CF_STONED) && !(ch[cn].flags&CF_PLAYER)) ch[cn].flags&=~CF_STONED;
	...
	if (((globs->flags & GF_MAYHEM) || globs->newmoon) && (ch[cn].flags & (CF_PLAYER)))
	{
		moonmult = 10;
	}
	if (globs->fullmoon && (ch[cn].flags & (CF_PLAYER)))
	{
		moonmult = 40;
	}
	...
	if ((in = ch[cn].worn[WN_NECK]) && it[in].temp==768) // amulet of ankh
	{
		if (ch[cn].skill[SK_REGEN][0])
		{
			ch[cn].a_hp += ch[cn].skill[SK_REGEN][5] * moonmult / 60;
		}
		if (ch[cn].skill[SK_REST][0])
		{
			ch[cn].a_end += ch[cn].skill[SK_REST][5] * moonmult / 60;
		}
		if (ch[cn].skill[SK_MEDIT][0])
		{
			ch[cn].a_mana += ch[cn].skill[SK_MEDIT][5] * moonmult / 60;
		}
	}
...
//                      ch[cn].data[92]=TICKS*60;
			if (it[in].flags & IF_PERMSPELL)
			{
				if (it[in].hp[0]!=-1)
				{
					ch[cn].a_hp += it[in].hp[0];
				}
				if (it[in].end[0]!=-1)
				{
					ch[cn].a_end += it[in].end[0];
				}
				if (it[in].mana[0]!=-1)
				{
					ch[cn].a_mana += it[in].mana[0];
				}
				if (ch[cn].a_hp<500)
				{
...
void do_shop_char(int cn, int co, int nr)
{
	...
	if ((in = ch[cn].citem)!=0 && (ch[co].flags & CF_MERCHANT))
	{
		...
		in2 = ch[co].data[0];
		if ((it[in].flags & IF_ARMOR) && (it_temp[in2].flags & IF_ARMOR))
		{
			flag = 1;
		}
...
void do_check_new_level(int cn)
{
	...
	if (ch[cn].data[45]<rank)
	{
		...
		if (diff==1)
		{
			do_char_log(cn, 0, "You rose a level! Congratulations! You received %d hitpoints, %d endurance and %d mana.\n",
			            hp * diff, end * diff, mana * diff);
		}
		else
		{
			do_char_log(cn, 0, "You rose %d levels! Congratulations! You received %d hitpoints, %d endurance and %d mana.\n",
