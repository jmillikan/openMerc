...
int friend_is_enemy(int cn, int cc)
{
	...
	if (!(co = ch[cn].attack_cn))
	{
		return( 0);
	}
	if (may_attack_msg(cc, co, 0))
	{
		return( 0);
	}
	return(1);
...
int spell_race_mod(int power, int kindred)
{
	...
	if (kindred & KIN_ARCHHARAKIM)
	{
		mod = 1.05;
	}
	else if (kindred & KIN_ARCHTEMPLAR)
	{
		mod = 0.95;
	}
	else if (kindred & KIN_SORCERER)
	{
		mod = 1.10;
	}
	else if (kindred & KIN_WARRIOR)
	{
		mod = 1.10;
	}
	else if (kindred & KIN_SEYAN_DU)
	{
		mod = 0.95;
	}
	else if (kindred & KIN_HARAKIM)
	{
		mod = 1.00;
	}
	else if (kindred & KIN_MERCENARY)
	{
		mod = 1.05;
	}
	else if (kindred & KIN_TEMPLAR)
	{
		mod = 0.90;
	}
	else
...
int add_spell(int cn, int in)
{
	...
	for (n = 0; n<20; n++)
	{
		if ((in2 = ch[cn].spell[n])!=0)
		{
			...
			{
				if (it[in].power<it[in2].power && it[in2].active>TICKS * 60)
				{
...
int is_exhausted(int cn)
{
	...
	for (n = 0; n<20; n++)
	{
		if ((in = ch[cn].spell[n])!=0 && it[in].temp==SK_BLAST)
		{
			break;
		}
	}
...
void add_exhaust(int cn, int len)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in add_exhaust");
		return;
	}
...
void spell_from_item(int cn, int in2)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_from_item");
		return;
	}
...
int spell_light(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_light");
		return( 0);
	}
	...
	if (cn!=co)
	{
		...
		{
			do_char_log(co, 1, "You start to emit light.\n");
		}
...
int spell_protect(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_protect");
		return( 0);
	}
	...
	if (cn!=co)
	{
		...
		{
			do_char_log(co, 0, "You feel protected.\n");
		}
...
int spell_enhance(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_enhance");
		return( 0);
	}
	...
	if (cn!=co)
	{
		...
		{
			do_char_log(co, 0, "Your weapon feels stronger.\n");
		}
...
int spell_bless(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_bless");
		return( 0);
	}
	...
	if (cn!=co)
	{
		...
		{
			do_char_log(co, 0, "You have been blessed.\n");
		}
...
void skill_wimp(int cn)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_wimp");
		return;
	}
...
int spell_mshield(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_mshield");
		return( 0);
	}
	...
	if (cn!=co)
	{
		...
		{
			do_char_log(co, 0, "Magic Shield active!\n");
		}
...
int spell_heal(int cn, int co, int power)
{
	if (cn!=co)
	{
		...
		{
			do_char_log(co, 0, "You have been healed.\n");
		}
...
int spell_curse(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in spell_curse");
		return( 0);
	}
...
int warcry(int cn, int co, int power)
{
	...
	if (ch[cn].attack_cn!=co && ch[co].alignment==10000)
	{
		return( 0);
	}

	// dont attack those we may not attack anyway
	if (!may_attack_msg(cn, co, 0))
	{
		return( 0);
	}

	// ignore those whose resistance is greater than our skill
	if (power<ch[co].skill[SK_RESIST][5])
	{
		return( 0);
	}
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_warcry");
		return( 0);
	}
...
void skill_identify(int cn)
{
	...
	if (chance_base(cn, ch[cn].skill[SK_IDENT][5], 18, power))
	{
		return;
	}
...
void skill_repair(int cn)
{
	...
	if (it[in].power)
	{
		chan = ch[cn].skill[SK_REPAIR][5] * 15 / it[in].power;
	}
	else
	{
		chan = 18;
...
void skill_recall(int cn)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_recall");
		return;
	}
...
int spell_stun(int cn, int co, int power)
{
	...
	if (!in)
	{
		xlog("god_create_item failed in skill_stun");
		return( 0);
	}
	...
	{
		do_char_log(co, 0, "You have been stunned.\n");
	}
...
void skill_stun(int cn)
{
	...
	if (cn==co)
	{
		do_char_log(cn, 0, "You cannot stun yourself!\n");
		return;
	}
...
void skill_dispel(int cn)
{
	...
	if ((co = ch[cn].skill_target1))
	{
		...
		{
			do_char_log(co, 0, "%s has been removed.\n", it[in].name);
		}
...
void skill_ghost(int cn)
{
	...
	{
		ch[cc].data[CHD_COMPANION] = globs->ticker + TICKS * 60 * 5; // NPC GCs stay only for 5 minutes
	}
...
//      ch[cn].errno=ERR_FAILED;        // will be overriden later if another result is desired
	...
	case    SK_REPAIR:
		...
		break;
	case    SK_LOCK:
		do_char_log(cn, 0, "You cannot use this skill directly. Hold a lock-pick under your mouse cursor and click on the door.\n");
		break;
	case    SK_RECALL:
		if (ch[cn].flags & CF_NOMAGIC)
		{
	...
	case    SK_IMMUN:
		do_char_log(cn, 0, "You use this skill automatically when someone casts evil spells on you.\n");
		break;

	case    SK_REGEN:
	case    SK_REST:
	case    SK_MEDIT:
		do_char_log(cn, 0, "You use this skill automatically when you stand still.\n");
		break;
