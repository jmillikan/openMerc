...
int so_login(unsigned char *buf, HWND hwnd)
{
	...
	if (buf[0]==SV_MOD2)
	{
		memcpy(mod + 15, buf + 1, 15);
		return( 0);
	}
	if (buf[0]==SV_MOD3)
	{
		memcpy(mod + 30, buf + 1, 15);
		return( 0);
	}
	if (buf[0]==SV_MOD4)
	{
		memcpy(mod + 45, buf + 1, 15);
		return( 0);
	}
	if (buf[0]==SV_MOD5)
	{
		memcpy(mod + 60, buf + 1, 15);
		return( 0);
	}
	if (buf[0]==SV_MOD6)
	{
		memcpy(mod + 75, buf + 1, 15);
		return( 0);
	}
	if (buf[0]==SV_MOD7)
	{
		memcpy(mod + 90, buf + 1, 15);
		return( 0);
	}
	if (buf[0]==SV_MOD8)
	{
...
int sv_setmap(unsigned char *buf, int off)
{
	...
	if (!buf[1])
	{
		DEBUG("Warning: no flags in setmap!");
		return( -1);
	}
...
void sv_exit(unsigned char *buf)
{
	...
	{
		xlog(1, "EXIT: Reason unknown.");
	}
	else
	{
		xlog(1, "EXIT: %s", logout_reason[reason]);
	}
...
int sv_cmd(unsigned char *buf)
{
	...
	case  SV_SETCHAR_AHP:
		sv_setchar_ahp(buf);
		return( 3);
	case  SV_SETCHAR_AEND:
		sv_setchar_aend(buf);
		return( 3);
	case  SV_SETCHAR_AMANA:
...
void xsend(unsigned char *buf)
{
	...
	while (len<16)
	{
		...
		if (ret<0)
		{
			...
			{
				so_error("transmit buffer overflow");
			}
			do_msg();
			continue;
		}
...
int tick_do(void)
{
	...
	while (idx<csize)
	{
		...
		if (ret==-1)
		{
			xlog(1, "Warning: syntax error in server data");
			DEBUG("Warning: syntax error in server data");
			exit(1);
		}
		idx += ret;
	}
