...
void add_words(void)
{
	...
	while (input[z1])
	{
		...
		buf[z2] = 0;
		add_word(buf);
		while (input[z1] && !xisalpha(input[z1]))
		{
			z1++;
		}
	}
...
/*           default:          xlog(3,"key=%d",(int)wParam);
	...
	case WM_CHAR:
		...
		default:
			if ((int)wParam>31 && (int)wParam<128 && in_len<115)
			{
				if (tabmode)
				{
					if (!isalnum((char)wParam))
					{
						in_len--;
					}
					cur_pos  = in_len;
...
void log_system_data(void)
{
	...
	GetUserName((void*)user, &size);
	size = 80;
	GetComputerName((void*)computer, &size);

	sprintf(buf, "|langid=%u, lcid=%u", langid, lcid);
	say(buf);
	sprintf(buf, "|systemdir=\"%s\"", systemdir);
	say(buf);
	sprintf(buf, "|windowsdir=\"%s\"", windir);
	say(buf);
	sprintf(buf, "|currentdir=\"%s\"", cdir);
	say(buf);
	sprintf(buf, "|username=\"%s\"", user);
	say(buf);
	sprintf(buf, "|computername=\"%s\"", computer);
	say(buf);
}
