...
void load_options(void)
{
	...
	if (handle!=-1)
	{
        ...
        if (read(handle, &hostdata, sizeof(hostdata))!=sizeof(hostdata))
        {
            //flag = 1;
        }
...
void save_options(void)
{
	...
	if (handle!=-1)
	{
        ...
        write(handle, &hostdata, sizeof(hostdata));
		close(handle);
	}
}
...
void load_char(HWND hwnd, char *name)
{
	...
	if (lseek(handle, 0, SEEK_END)>(long) (sizeof(struct pdata) + sizeof(struct key)))
	{
		flag = 1;
	}
...
void update_race(HWND hwnd)
{
	if (race==1)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_TEMP);
	}
	if (race==2)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_MERC);
	}
	if (race==3)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_HARA);
	}
	if (race==4)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_SEY);
	}
	if (race==5)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_ARCHTEMPLAR);
	}
	if (race==6)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_ARCHHARAKIM);
	}
	if (race==7)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_WARRIOR);
	}
	if (race==8)
	{
		CheckRadioButton(hwnd, IDC_TEMP, IDC_SEY, IDC_SORCERER);
	}

	if (sex==1)
	{
		CheckRadioButton(hwnd, IDC_MALE, IDC_FEMALE, IDC_MALE);
	}
	if (sex==2)
	{
		CheckRadioButton(hwnd, IDC_MALE, IDC_FEMALE, IDC_FEMALE);
	}
}
...
void update_alpha(HWND hwnd)
{
	if (do_alpha==2)
	{
		CheckRadioButton(hwnd, IDC_FULLALPHA, IDC_NOALPHA, IDC_FULLALPHA);
	}
	if (do_alpha==1)
	{
		CheckRadioButton(hwnd, IDC_FULLALPHA, IDC_NOALPHA, IDC_PARTIALALPHA);
	}
	if (do_alpha==0)
	{
		CheckRadioButton(hwnd, IDC_FULLALPHA, IDC_NOALPHA, IDC_NOALPHA);
	}
}
...
APIENTRY OptionsProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		...
		//else domusic=0;
		if (IsDlgButtonChecked(hwnd, IDC_DOSOUND))
		{
			dosound = 1;
		}
		else
		...
		if (IsDlgButtonChecked(hwnd, IDC_DOSHADOW))
		{
			do_shadow = 1;
		}
		else
	...
	case WM_COMMAND:
		...
		case    IDC_NEW:
			...
			return(1);
		case IDC_SAVE:
			if (save_dialog(hwnd, buf))
			{
				save_char(hwnd, buf);
			}
			return(1);
		case IDC_LOAD:
			if (load_dialog(hwnd, buf))
			{
				load_char(hwnd, buf);
			}
			SetDlgItemText(hwnd, IDC_RNAME, okey.name);
		...
		case IDC_MALE:
			if (okey.usnr)
			{
				MessageBox(hwnd, "Changing the gender has no effect for existing accounts.", "Sorry", MB_OK | MB_ICONSTOP);
			}
			else
			{
				sex = 1;
